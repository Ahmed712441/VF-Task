...
export class CoinGeckoService {
  ...
  async getMarketsData(
    ...
    try {
      const data = await this.http.get<CryptoMarketData[]>(
        "/coins/markets",
        params,
      );
      return data;
  ...
  async getCryptoHistoricalDataById(
    ...
    const { vsCurrency = "usd", days = "1" } = options;
    ...
    try {
      const data = await this.http.get<CryptoHistoricalData>(
